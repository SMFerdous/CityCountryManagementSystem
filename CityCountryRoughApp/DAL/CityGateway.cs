...
namespace CityCountryRoughApp.DAL
{
    public class CityGateway
    {
        ...
        public List<CountryViewModel> GetAllCities()
        {
            ...
            string query = "SELECT ISNULL(ci.Id,0) as Id ,ISNULL(ci.Name,0) as Name,ISNULL(ci.noOfDwellers,0) as NoOfDwellers ,co.Name as CountryName From Cities as ci LEFT OUTER JOIN Countries as co ON ci.CountryId = co.Id Order By ci.Name ";
         ...
         public  List<City> GetAllCitiesForView()
       {
        ...
        //wait yahoo
         public List<City> GetSearchByCity(string name)

            {
             ...
             string query = "SELECT ci.Id as [Id], ci.Name as [Name],ci.About as [About],ci.noOfDwellers as [NoOfDwellers],ci.Location as [Location],ci.Weather as [Weather],ISNULL(co.Name,0) as [CountryName] ,ISNULL(co.About,0)  as [CountryAbout] From Cities as ci LEFT OUTER Join Countries as co ON ci.CountryId = co.Id  WHERE  ci.Name like '%' + @name + '%' ORDER BY ci.Name";
        ...
        //moja paici
         public List<CountryViewModel> GetSearchByCountryForCityView(string name)
         {
             ...
             SqlConnection connection = new SqlConnection(connectionString);
             string query = "SELECT ISNULL(ci.Id,co.Id)  as[Id],  ISNULL(ci.Name,0) as[Name], ISNULL(ci.About,0)  as [About],ISNULL(ci.noOfDwellers,0) as[noOfDwellers], ISNULL(ci.Location,0)  as [Location] ,ISNULL(ci.Weather,0)  as [Weather],co.Id, co.Name as CountryName , co.About as CountryAbout From Cities as ci RIGHT OUTER Join Countries as co ON ci.CountryId = co.Id  WHERE  co.Name LIKE '%' + @name + '%' ORDER BY co.Name";
             ...
             while (reader.Read())
             {
                 ...
                 //citiesCountryViewModel.CountryId = Convert.ToInt32(reader["CountryId"].ToString());
         ...
         public List<Country> GetAllCountriesDropForViewCity()
         {
             ...
             string query = " SELECT co.Id as Id,co.Name as CountryName,co.About as CountryAbout FROM COUNTRIES as co ORDER BY co.Name ";
